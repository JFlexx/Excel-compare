...
function normalizeDecisionActor(decidedBy) {
  if (!decidedBy) {
    return {
      userId: 'user:unknown',
  ...
  return {
    userId: decidedBy.userId ?? 'user:unknown',
    displayName: decidedBy.displayName ?? decidedBy.userId ?? 'Usuario interno',
    origin: decidedBy.origin ?? 'office-addin'
  };
}

function buildDecisionHistoryEntry({ sessionId, conflict, targetId, decision, finalValue, decidedAt, decidedBy }) {
  return {
    id: `history:${targetId}:${decidedAt}:${decision}`,
    ...
    actor: normalizeDecisionActor(decidedBy),
    changeOrigin: 'manual',
    location: {
      worksheetName: conflict.location?.worksheetName ?? null,
      a1: conflict.location?.a1 ?? null,
      rangeA1: conflict.location?.rangeA1 ?? conflict.location?.a1 ?? null
    }
  };
}
...
export function createManualEditDecision({ conflict, rawValue, decidedBy, decidedAt = new Date().toISOString(), sessionId }) {
